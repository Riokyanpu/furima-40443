class User < ApplicationRecord
  ...
  validates :surname_kana, presence: true, format: { with: /\A[ァ-ヶー]+\z/, message: "はカタカナのみ使用できます" }
  validates :name, presence: true, format: { with: /\A[ぁ-んァ-ヶ一-龥々ー]+\z/, message: "はひらがな、カタカナ、漢字のみ使用できます" }
  validates :name_kana, presence: true, format: { with: /\A[ァ-ヶー]+\z/, message: "はカタカナのみ使用できます" }
